...
def test_optimized_folder_upload_with_tables():
    ...
    for pdf_path in test_files:
        if os.path.exists(pdf_path):
            ...
            print(
                f"✅ Found: {pdf_path} ({os.path.getsize(pdf_path) / (1024*1024):.2f} MB)")
        else:
    ...
    for pdf_path in existing_files:
        files.append(('files', (os.path.basename(pdf_path),
    ...
    try:
        ...
        print(
            f"   Total size: {sum(os.path.getsize(f) for f in existing_files) / (1024*1024):.2f} MB")
        ...
        if response.status_code == 200:
            ...
            print(
                f"   Total time: {processing_time:.2f}s ({processing_time/60:.2f} minutes)")
            print(
                f"   Server time: {result.get('total_time_ms', 0)/1000:.2f}s")
            ...
            if not target_met:
                print(
                    f"   Exceeded by: {(processing_time - 240)/60:.2f} minutes")
            ...
            if outputs:
                ...
                for i, output in enumerate(outputs, 1):
                    ...
                    print(f"   {i}. {pdf_name}")
                    print(
                        f"      Pages: {pages} | Time: {proc_time:.2f}s | Table workers: {table_workers}")
                    ...
                    if json_path:
                        ...
                        if os.path.exists(full_json_path):
                            ...
                            try:
                                ...
                                table_count = sum(
                                    len(page.get('tables', [])) for page in pages_data)
                                print(
                                    f"      Tables extracted: {table_count} ✅")
            ...
            if result.get('total_pages', 0) > 0:
                pages_per_minute = result.get(
                    'total_pages', 0) / (processing_time / 60)
                print(f"\n📈 Performance Metrics:")
                print(f"   Pages per minute: {pages_per_minute:.1f}")
                print(
                    f"   Seconds per page: {processing_time / result.get('total_pages', 1):.2f}")

                # Estimate scalability
                # seconds for 100 pages
                estimated_100_pages = (100 / pages_per_minute) * 60
                print(
                    f"   Estimated time for 100 pages: {estimated_100_pages/60:.2f} minutes")
