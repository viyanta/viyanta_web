...
def test_table_extraction():
    ...
    try:
        ...
        result = extract_pdf_to_json(
            test_pdf, test_output_dir, skip_tables=False)
        ...
        if json_path and os.path.exists(os.path.join("..", json_path)):
            ...
            print(
                f"📃 Pages with tables: {pages_with_tables}/{len(data.get('pages', []))}")
            ...
            if total_tables > 0:
                ...
                # Check first 3 pages
                for i, page in enumerate(data.get('pages', [])[:3]):
                    page_tables = page.get('tables', [])
                    if page_tables:
                        print(
                            f"   Page {page.get('page_number', i+1)}: {len(page_tables)} table(s)")
                ...
                print(
                    "⚠️  No tables found - this might be normal if the PDF has no tables")
