...
def extract_metadata(text, template):
    ...
    title_raw = company_match.group(1) if company_match else template.get("Title", "")
    ...
    meta["RegistrationNumber"] = reg_match.group(1).strip() if reg_match else ""
...
def extract_tables_from_page(pdf_path, page_number):
    ...
    try:
        ...
        camelot_tables = camelot.read_pdf(pdf_path, pages=str(page_number), flavor="stream")
        ...
        for i, t in enumerate(camelot_tables):
            ...
            if table_data and len(table_data) > 0:
                print(f"Camelot table {i}: {len(table_data)} rows, {len(table_data[0]) if table_data else 0} cols")
                tables.append(table_data)
        
        # If stream didn't find good tables, try lattice
        if not tables:
            print("[INFO] Trying camelot lattice...")
            camelot_tables = camelot.read_pdf(pdf_path, pages=str(page_number), flavor="lattice")
            print(f"Camelot lattice found {len(camelot_tables)} tables")
            
            for i, t in enumerate(camelot_tables):
                table_data = t.df.values.tolist()
                if table_data and len(table_data) > 0:
                    print(f"Camelot lattice table {i}: {len(table_data)} rows, {len(table_data[0]) if table_data else 0} cols")
                    tables.append(table_data)
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            if page_number <= len(pdf.pages):
                ...
                if tb and len(tb) > 0:
                    print(f"[SUCCESS] PDFplumber structured table: {len(tb)} rows, {len(tb[0]) if tb else 0} cols")
                ...
                if all_tables:
                    print(f"[SUCCESS] PDFplumber found {len(all_tables)} tables")
                    for i, table in enumerate(all_tables):
                        if table and len(table) > 0:
                            print(f"PDFplumber table {i}: {len(table)} rows, {len(table[0]) if table else 0} cols")
                            # Only add if we don't already have this data
                ...
                if not tables:
                    ...
                    if text:
                        ...
                        if text_table:
                            print(f"[SUCCESS] Text-based table: {len(text_table)} rows")
                            tables.append(text_table)
                            
            else:
                print(f"[ERROR] Page {page_number} does not exist (total pages: {len(pdf.pages)})")
    ...
    for existing in existing_tables:
        ...
        if abs(new_rows - existing_rows) <= 1 and abs(new_cols - existing_cols) <= 1:
            ...
            if new_table and existing:
                try:
                    new_first = str(new_table[0][0] if new_table[0] else "").strip().lower()
                    existing_first = str(existing[0][0] if existing[0] else "").strip().lower()
...
def _extract_table_from_text(text):
    ...
    for line in lines:
        ...
        if (re.search(r'\d', line) or 
            ...
            cols = re.split(r'\s{2,}|\t+|(?<=\d)\s+(?=[A-Za-z])|(?<=[a-z])\s+(?=\d)', line)
...
def map_to_rows(table, flat_headers, meta):
    ...
    print(f"Mapping table with {len(table)} rows to {len(flat_headers)} headers: {flat_headers}")
    ...
    print(f"Header row detected at index: {header_row_idx}, data starts at: {data_start_idx}")
    
    # Process data rows
    data_rows = all_rows[data_start_idx:] if data_start_idx < len(all_rows) else all_rows
    ...
    for row_idx, row in enumerate(data_rows):
        ...
        print(f"Processing row {row_idx}: {str(cleaned[:3]).encode('ascii', 'ignore').decode('ascii')}..." if len(cleaned) > 3 else f"Processing row {row_idx}: {str(cleaned).encode('ascii', 'ignore').decode('ascii')}")
        ...
        if _is_valid_data_row(mapped, header_tokens, meta_values_norm):
            ...
            print(f"Added row {row_idx}: {str(mapped.get(flat_headers[0], '')).encode('ascii', 'ignore').decode('ascii')}")
        else:
...
def _map_row_to_headers(cleaned_cells, flat_headers):
    ...
    if len(cleaned_cells) > len(flat_headers):
        print(f"More cells ({len(cleaned_cells)}) than headers ({len(flat_headers)}), adjusting...")
...
def _is_valid_data_row(mapped_row, header_tokens, meta_values_norm):
    ...
    if header_like_count >= len(non_empty) * 0.7:  # 70% threshold instead of strict rules
        return False
...
def is_table_of_contents_page(text):
    ...
    form_pattern_matches = len(re.findall(r"\bL-\d+[A-Z]*\b", text, re.IGNORECASE))
    ...
    has_list_structure = bool(re.search(r'\.\.\.\.*\s*\d+|page\s+\d+|\d+\s*\.\s*\d+', text_lower))
...
def extract_form(pdf_path, template_json, output_json):
    print(f"Starting extraction: PDF={pdf_path}, Template={template_json}, Output={output_json}")
    ...
    try:
        ...
        normalized_headers = [normalize_header_for_display(h) for h in flat_headers]
        results = []
        ...
        with pdfplumber.open(pdf_path) as pdf:
            ...
            if pdf.pages:
                ...
                adjusted_headers = _get_headers_for_form_type(form_type, flat_headers)
            ...
            for page_idx, page in enumerate(pdf.pages, start=1):
                ...
                if len(text) < 20:
                    print(f"Page {page_idx} has very little text ({len(text)} chars), but still processing...")
                else:
                ...
                if is_table_of_contents_page(text):
                    print(f"Skipping page {page_idx} - detected as table of contents")
                    continue

                meta = extract_metadata(text, template)
                form_no = meta.get("Form No", "")
                print(f"Extracted metadata: Form No='{form_no}', Title='{meta.get('Title', '')}', Currency='{meta.get('Currency', '')}'")
                ...
                for table_idx, table in enumerate(tables):
                    ...
                    print(f"Table dimensions: {len(table)} rows x {len(table[0]) if table else 0} cols")
                    
                    rows = map_to_rows(table, flat_headers, meta)
                    table_row_count = len(rows)
                    page_total_rows += table_row_count
                    
                    print(f"Table {table_idx + 1} produced {table_row_count} data rows")
                    ...
                    if rows:
                        ...
                        print(f"[OK] Added {table_row_count} rows from table {table_idx + 1}")

                print(f"Page {page_idx} summary: {len(tables)} tables processed, {page_total_rows} total rows extracted")
                
                # If no data was found but we had tables, show a warning
                if tables and not page_has_data:
                    print(f"[WARNING] Page {page_idx} had {len(tables)} tables but no data rows extracted!")
        ...
        if not results:
            print("[WARNING] No data extracted from any page! This might indicate an extraction issue.")
