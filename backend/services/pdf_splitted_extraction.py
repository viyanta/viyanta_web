...
def load_template(path):
    with open(path, "r", encoding="utf-8") as f:
        ...
        if "Headers" in template:
            ...
            print(f"Extracted keys from Headers: {flat_headers}")
            template["FlatHeaders"] = flat_headers
...
def extract_tables_from_page(pdf_path, page_number):
    ...
    print("[INFO] Using pdfplumber for table extraction...")
    try:
        with pdfplumber.open(pdf_path) as pdf:
            if page_number <= len(pdf.pages):
                ...
                if tb:
                    print(
                        f"[SUCCESS] PDFplumber table: {len(tb)} rows, {len(tb[0]) if tb else 0} cols")
                    tables.append(tb)
                else:
                    print("[WARNING] No structured table found with pdfplumber")

                    # If no table, try to extract text in a structured way
                    text = page.extract_text()
                    if text:
                        print(
                            f"[INFO] Extracting data from text content ({len(text)} chars)")
                        ...
                        for line in lines:
                            ...
                            if (re.search(r'\d', line) or
                                    any(keyword in line.lower() for keyword in ['commission', 'total', 'gross', 'net', 'premium', 'expense', 'particulars'])):
                        ...
                        if potential_table:
                            print(
                                f"[SUCCESS] Extracted text-based table: {len(potential_table)} rows")
                            tables.append(potential_table)
                        else:
                            print("[WARNING] No tabular data found in text")

                            # Last resort: create a simple structure from all text
                            print("[INFO] Creating basic text extraction...")
                            text_rows = []
                            for line in lines:
                                line = line.strip()
                            ...
                            if text_rows:
                                ...
                                print(
                                    f"[SUCCESS] Created text-based extraction: {len(text_rows)} lines")
                    else:
                        print("[ERROR] No text found on page")
            else:
                print(
    ...
    if not tables:
        print("[INFO] Falling back to camelot...")
        try:
            ...
            print(f"[ERROR] Camelot extraction failed: {e}")
...
def map_to_rows(table, flat_headers, meta, form_no: str, page_idx: int):
    ...
    for r_idx, row in enumerate(data_rows):
        ...
        print(f"Processing row {r_idx}: {str(cleaned_cells[:3]).encode('ascii', 'ignore').decode('ascii')}..." if len(
            cleaned_cells) > 3 else f"Processing row {r_idx}: {str(cleaned_cells).encode('ascii', 'ignore').decode('ascii')}")
        ...
        print(f"Added row {r_idx}: {str(first_col_value).encode('ascii', 'ignore').decode('ascii')}")
