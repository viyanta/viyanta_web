...
"""
    ...
    def _extract_partial_data(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            accuracy_match = re.search(r'"accuracy_score":\s*(\d+)', response_text)
            accuracy_score = 85
            if accuracy_match:
                accuracy_score = int(accuracy_match.group(1))
            
            # Extract corrections made
            corrections_match = re.search(r'"corrections_made":\s*\[(.*?)\]', response_text, re.DOTALL)
            corrections = []
            if corrections_match:
                corrections_text = corrections_match.group(1)
                # Extract individual corrections
                correction_items = re.findall(r'"([^"]+)"', corrections_text)
                corrections = [item for item in correction_items if not item.endswith('...') and len(item) > 10]
            ...
            message = message_match.group(1) if message_match else "Verification completed with partial data extraction"
            ...
            logger.error(f"Error extracting partial data: {e}")
            return self._create_fallback_response()
