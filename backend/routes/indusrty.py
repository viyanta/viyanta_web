...
# Get Selected Row IDs for Dashboard
...
# Update Selected Row IDs for Dashboard
...
def update_selected_row_ids(request: UpdateSelectedRowIdsRequest, db=Depends(get_db)):
    ...
    try:
        ...
        result = db.execute(check_query, {
            "data_type": data_type_with_prefix,
...
def get_selected_descriptions(db=Depends(get_db)):
    ...
    try:
        ...
        if DB_TYPE == "mysql":
            create_table_query = text("""
                CREATE TABLE IF NOT EXISTS dashboard_selected_descriptions_industry (
                    id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    try:
        ...
        for desc in descriptions:
            if desc and desc not in current_descriptions:
                if DB_TYPE == "mysql":
                    insert_query = text("""
                        INSERT INTO dashboard_selected_descriptions_industry (description)
                        VALUES (:description)
                        ON DUPLICATE KEY UPDATE description = :description
                    """)
                else:
                    insert_query = text("""
                        INSERT OR IGNORE INTO dashboard_selected_descriptions_industry (description)
                        VALUES (:description)
                    """)
