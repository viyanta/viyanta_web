...
const IndustryMetricsDomestic = ({ onMenuClick }) => {
  ...
  useEffect(() => {
    const fetchDescriptions = async () => {
      ...
      try {
        ...
        const descriptions = await ApiService.getDescriptionsIndustry('Domestic', selectedPremiumType, selectedCategory);
  ...
  const fetchIndustryData = useCallback(async () => {
    ...
    try {
      ...
      if (selectedDescription && selectedDescriptions.includes(selectedDescription)) {
        try {
          const selectedIds = await ApiService.getSelectedRowIdsIndustry('Domestic', selectedDescription);
  ...
  const handleRowSelection = async (rowId, isSelected) => {
    try {
      ...
      await ApiService.updateSelectedRowIdsIndustry('Domestic', selectedDescription, rowIdsArray);
  ...
  const handleSelectAll = async (selectAll) => {
    try {
      if (selectAll) {
        ...
        await ApiService.updateSelectedRowIdsIndustry('Domestic', selectedDescription, []);
  ...
  const fetchUniqueValues = useCallback(async () => {
    try {
      ...
      for (const field of fields) {
        try {
          const data = await ApiService.getUniqueValuesIndustry('Domestic', field);
          values[field] = data || [];
        } catch (err) {
  ...
  const handleEdit = async (record) => {
    ...
    if (premiumTypeValue && isPremiumTypeInList) {
      try {
        const categoryData = await ApiService.getCategoriesIndustry('Domestic', premiumTypeValue);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        const updatedPremiumTypes = await ApiService.getPremiumTypesIndustry('Domestic');
      ...
      if (selectedPremiumType || formData.PremiumTypeLongName) {
        try {
          ...
          const updatedCategories = await ApiService.getCategoriesIndustry('Domestic', premiumTypeToUse);
                ...
                <select
                  ...
                  onChange={async (e) => {
                    ...
                    if (value === '__ADD_NEW__') {
                      ...
                      try {
                        const categoryData = await ApiService.getCategoriesIndustry('Domestic', value);
                        setModalCategories(categoryData || []);
                        setSelectedCategoryOption('');
