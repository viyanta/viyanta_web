...
function Explorer() {
    ...
    // New state for folder upload
    const [mode, setMode] = React.useState('file'); // 'file' | 'folder'
    const [folderFiles, setFolderFiles] = React.useState([]);
    const [folderResult, setFolderResult] = React.useState(null);
    const [folderName, setFolderName] = React.useState('');

    // Folder browsing state
    const [selectedPdf, setSelectedPdf] = React.useState(null);
    const [selectedPdfJson, setSelectedPdfJson] = React.useState(null);
    const [selectedPage, setSelectedPage] = React.useState(null);
    const [jsonLoading, setJsonLoading] = React.useState(false);
    const [jsonError, setJsonError] = React.useState(null);

    // New state for extraction mode
    const [extractMode, setExtractMode] = React.useState('fast'); // 'fast' | 'complete'
              ...
              <div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem' }}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <input
                    ...
                    onChange={(e) => setExtractMode(e.target.value)}
                  />
                  <span>📊 <strong>Complete</strong> (Text + Tables - 5-15min)</span>
                </label>
              </div>
            </div>
            <div className="form-group">
