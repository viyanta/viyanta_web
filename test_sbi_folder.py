...
def test_optimized_folder_upload():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Server reported time: {result.get('total_time_ms', 0)/1000:.2f}s")
            ...
            if total_time <= 120:  # 2 minutes
                print(f"   🎉 TARGET MET: Completed in {total_time:.1f}s (< 2 minutes) ✅")
            else:
                print(f"   ⚠️ TARGET MISSED: Took {total_time:.1f}s (> 2 minutes) ❌")
            ...
            if outputs:
                ...
                for i, output in enumerate(outputs[:3]):  # Check first 3 files
                    ...
                    processing_time = output.get('processing_time_ms', 0) / 1000
